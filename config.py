...
# === API ТОКЕНЫ (берутся из .env файла) ===
API_TOKEN = os.getenv("BOT_TOKEN")              # Токен Telegram бота
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")    # Ключ OpenAI API

# === НАСТРОЙКИ СЕМАНТИЧЕСКОГО АНАЛИЗА ===

# Порог схожести для отбора релевантных страниц (0.0-1.0, чем выше - тем строже отбор)
SEMANTIC_SCORE_THRESHOLD = 0.5

# Максимальное количество страниц для анализа (ограничивает расходы на API)
SEMANTIC_TOP_PAGES_LIMIT = 10

# === ПОИСКОВЫЕ ФРАЗЫ ДЛЯ СЕМАНТИЧЕСКОГО АНАЛИЗА ===

# Примеры фраз о дефектах для обучения семантического роутера
# (используются для поиска релевантных страниц с дефектами)
...
# Модель для Vision Language Model (очистка текста со страниц)
VLM_MODEL = "gpt-4.1-mini"

# Модель для анализа дефектов (основной LLM анализ)
